...
@dataclass
class NewtonsMethod(ComparingIteratedFunctionWithCombinator[PyTree, Array, Array, Array]):

    f: Callable[[PyTree, Array], Array] = field(static=True)
    step_size: float

    # Implemented methods --------------------------------------------------------------------------
    def iterate_state(self, theta: PyTree, state: Array) -> Array:
...
class NoisyNewtonsMethod(StochasticIteratedFunctionWithCombinator[PyTree, State, Array, Array]):
    ...
    # Implemented methods --------------------------------------------------------------------------
    def iterate_state(self, theta: PyTree, state: State) -> State:
        x, rng = state
        # https://github.com/python/mypy/issues/5485
    ...
    def stochastic_iterate_state(self, theta: PyTree, state: State) -> State:
        new_x, rng = self.iterate_state(theta, state)
...
def fixture_fixed_point_using_while(it_fun: IteratedFunction[Array, Array, Array, Array]) -> C:
...
def fixture_fixed_point_using_scan(it_fun: IteratedFunction[Array, Array, Array, Array]) -> C:
    def f(theta: Array, x_init: Array) -> Array:
...
def test_noisy_grad(noisy_it_fun: IteratedFunction[Array, Array, Array, Array],
                    theta: float) -> None:
