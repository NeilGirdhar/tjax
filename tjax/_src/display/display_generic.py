...
def _(
    ...
    *,
    ...
    # No need for seen set ellision.
    retval = _assemble(
        key, Text(f"NumPy Array {value.shape} {value.dtype}", style=_numpy_array_color)
    )
...
@display_generic.register
def _(
    value: Array | DynamicJaxprTracer,
    *,
    seen: MutableSet[int] | None = None,
    key: str = "",
) -> Tree:
    # No need for seen set ellision.
    retval = _assemble(key, Text(f"Jax Array {value.shape} {value.dtype}", style=_jax_array_color))
    try:
...
@contextmanager
def _verify(value: object, seen: MutableSet[int], key: str) -> Generator[Tree | None]:
    if id(value) in seen:
...
def _assemble(key: str, type_text: Text, separator: str = "=") -> Tree:
    ...
    if key:
        return Tree(
            Text.assemble(
                Text(key, style=_key_color), Text(separator, style=_separator_color), type_text
            )
        )
...
def _show_array(tree: Tree, array: NumpyArray) -> None:
    ...
    if len(array.shape) == 0:
        ...
        return
    table = Table(show_header=False, show_edge=False, style=_table_color)
